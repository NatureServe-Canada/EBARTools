...
class PackageECCCPrioritySpeciesTool:
    ...
    def processPackage(self, messages, range_map_ids, attributes_dict, zip_folder, metadata):
        ...
        shutil.copyfile(EBARUtils.resources_folder + '/EBAR.mxd', zip_folder + '/EBAR_ECCC_2022-23_Birds.mxd')
        ...
        EBARUtils.displayMessage(messages, 'Creating ZIP: https://gis.natureserve.ca/download/' +
                                 'EBAR_ECCC_2022-23_Birds_All_Data.zip')
        EBARUtils.createZip(zip_folder,
                            EBARUtils.download_folder + '/EBAR_ECCC_2022-23_Birds_All_Data.zip', None)
        # EBARUtils.displayMessage(messages, 'Creating ZIP: https://gis.natureserve.ca/download/' +
        ...
        # create pdf zip
        EBARUtils.displayMessage(messages, 'Creating ZIP: https://gis.natureserve.ca/download/' +
                                 'EBAR_ECCC_2022-23_Birds_All_PDFs.zip')
        EBARUtils.createZip(zip_folder,
                            EBARUtils.download_folder + '/EBAR_ECCC_2022-23_Birds_All_PDFs.zip','.pdf')
        # EBARUtils.displayMessage(messages, 'Creating ZIP: https://gis.natureserve.ca/download/' +
    ...
    def runPackageECCCPrioritySpeciesTool(self, parameters, messages):
        ...
        arcpy.MakeTableView_management(EBARUtils.ebar_feature_service + '/11', 'range_map_view',
                                       'RangeMapID IN (3047,2977,3087,3038,2707,2708,3089,2709,3050,2993,2693,3002,2712,3112,2714,2715,2700,3003,3004,3106,2704,2969,2716,2717,3117,2718,2719,2720,3031,3104,3105,3006,2723,2970,2971,2972,2975,2759,2760,2761,3092,2763,3093,2764,3094,3080,3009,3120,3010,3011,2765,3240,3013,2766,3032,3014,3015,3016,2768,3041,2994,3097,3098,2770,2997,2772,2773,2774,2728,2730,3018,2733,2735,2738,2739,2740,3082,3083,3042,2742,2745,2974,2748,2751,2979,2695,2694,2976,3033,3034,2757,2657,2775,2980,2776,3023,2658,3025,2777,2778)')
        ...
        # make zip folder
        zip_folder = EBARUtils.temp_folder + '/EBAR_ECCC_2022-23_Birds_All_Data'
